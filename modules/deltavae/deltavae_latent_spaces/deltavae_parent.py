    ...
    def _build_loss_functions(self, z_log_scale, z_mean_projected):
        ...
        def r_loss(inputs, outputs):
            ...
            if self.r_loss == "mse":
                ...
                epsilon = K.epsilon()
                loss = -inputs.cpu().numpy().astype(np.float32) * tf.math.log(epsilon + outputs).cpu().numpy().astype(np.float32) \
                       - (1 - inputs).cpu().numpy().astype(np.float32) * tf.math.log(epsilon + 1 - outputs).cpu().numpy().astype(np.float32)
                # Sum over the data dimensions
                for dimension in range(len(self.encoder_class.input_shape)):
